...
def _copy_file_with_open(src, dst):
...
def copy_file_with_retries(src, dst, retries=10, delay=0.5):
    src = Path(src)
    dst = Path(dst)
...
def create_split_dataset(dataset_root, pairs, train_ratio=0.7, val_ratio=0.15, test_ratio=0.15):
    ...
    for subset, batch in subsets:
        for image_path, label_path in batch:
            ...
            copy_file_with_retries(image_path, image_dest)
            copy_file_with_retries(label_path, label_dest)
...
def main():
    ...
    # Determine next incremental model folder name: ping_modelv{n}
    import re
    existing = []
    # check existing folders and zip files in cwd
    existing += glob.glob(os.path.join(os.getcwd(), "ping_modelv*"))
    existing += glob.glob(os.path.join(os.getcwd(), "ping_modelv*.zip"))
    max_idx = 0
    for p in existing:
        base = os.path.basename(p)
        m = re.search(r"ping_modelv(\d+)", base)
        if m:
            try:
                idx = int(m.group(1))
                if idx > max_idx:
                ...
                pass
    next_idx = max_idx + 1
    run_name = f"ping_modelv{next_idx}"

    # Train with the incremental run name so Ultralytics writes to runs/.../{run_name}
    model.train(data=str(split_data_yaml), epochs=100, name=run_name, device=train_device, batch=4)

    # Create output directory to store model weights and train results
    output_dir = os.path.join(os.getcwd(), run_name)
    os.makedirs(output_dir, exist_ok=True)

    # Find run folder(s) that match the chosen run_name under runs/**/run_name
    run_candidates = glob.glob(os.path.join("runs", "**", run_name), recursive=True)
    latest_run = None
    if run_candidates:
        # copy all matching run folders (usually one)
        for run_path in run_candidates:
            dest = os.path.join(output_dir, os.path.basename(run_path))
            try:
                shutil.copytree(run_path, dest, dirs_exist_ok=True)
            except Exception as e:
                print(f"Warning: failed to copy training run {run_path}: {e}")
        latest_run = run_candidates[-1]
    else:
        # As a fallback, look for any recent runs and copy those (older behavior)
    ...
    # Zip into '{run_name}.zip'
    zip_base = os.path.join(os.getcwd(), run_name)
